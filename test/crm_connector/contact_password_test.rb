...
module Infopark; module Crm
  ...
  class ContactPasswordTest < ConnectorTestCase
    ...
    def test_password_set_sends_password_in_post_body_only
      WebMock.enable!
      stub_request(:post, %r{http://.*/api/contacts/password_set[^X]*$}).
        with(:body => {
    ...
    def test_authenticate_sends_password_in_post_body_only
      WebMock.enable!
      stub_request(:post, %r{http://.*/api/contacts/authenticate[^X]*$}).
        with(:body => {
            'login' => 'the_X_login',
            'password' => 'the_password'
            }).to_return(:status => 200, :body => '{"id":"12345678"}', :headers => {})
      stub_request(:get, %r{http://.*/api/contacts/12345678.json}).
    ...
    def test_password_request_sends_params_in_post_body_only
      WebMock.enable!
      stub_request(:post, %r{http://.*/api/contacts/.*/password_request[^X]*$}).
        with(:body => {
            'only_get_token' => true,
